/* ==============================================================
   AGENTE GEMINI – Studio 118 (v3 – Optimizado para bajo coste)
   --------------------------------------------------------------
...
... (el mismo texto que tenías antes) ...

---
### 🚫 REGLAS TÉCNICAS
- Adaptas tu lenguaje al de la clienta.
- No inventas disponibilidad.
...
try {
  ...
  console.error("⚠️  No se pudo cargar precios.json → usar precios fijos internos");
  PRECIOS = { color: 45, extensiones: 120, nanoplastia: 80, corte: 35 };
}
...
async function getModelName(lowCost = true) {
  ...
  if (!cachedAvailableModels) {
    ...
    cachedAvailableModels = (data.models || []).map(m => m.name.replace(/^models\//, ""));
  }
  ...
  if (!chosen) throw new Error(`Ningún modelo preferido disponible: ${preferred.join(", ")}`);
...
const SYSTEM_ACK = {
  role: "model",
  parts: [{ text: "Entendido. Estoy lista para ayudar a que nuestras clientas brillen bonito. ✨" }],
};
...
const FUNCTION_DEFINITIONS = [
  ...
  },
  {
    ...
    description: "Comprueba si existe al menos un hueco libre para la fecha y hora indicadas.",
    parameters: {
      type: "object",
      properties: {
        fechaISO: {
          type: "string",
          format: "date-time",
          description: "Fecha y hora en ISO 8601 (ej.: 2025-10-04T15:30:00.000Z)",
        },
        servicio: {
          type: "string",
...
function normalizeAlternatingHistory(history) {
  ...
  for (const m of history) {
    ...
    if (last.role === m.role) {
      ...
      out[out.length - 1] = { role: last.role, content: newContent, media: newMedia };
    } else {
      out.push({ ...m, media: m.media || [] });
    }
  }
...
function sanitizeGeminiResponse(raw) {
  ...
  if (clean.toLowerCase().includes("cita") && !CONFIRMATION_REGEX.test(clean)) {
  ...
  if (result.functionCall) {
    ...
    if (name === "obtenerPrecioServicio") {
      fnResult = {
        ...
        precioUSD: PRECIOS[args.servicio] ?? null,
        detalle:
      ...
      fnResult = {
        disponible: true,
        mensaje: "Hay espacios libres en la fecha y hora solicitada. Puedes confirmar la cita.",
      };
    }
